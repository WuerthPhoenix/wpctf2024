...
CHALLENGE_NAME="secure-file-scanner"
...
FLAG=$(cat flag.txt)
"$CONTAINER_RUNTIME" build --file Dockerfile --build-arg FLAG="$FLAG" --no-cache --tag wpctf2024-challenges:"$CHALLENGE_NAME" .
...
"$CONTAINER_RUNTIME" cp "$tmp_container_id":/app/scanner .
...
zip "$CHALLENGE_NAME".zip scanner
